...
describe('@rule.js/contextualize', () => {
  describe('equal', () => {
    const isFoo = Rule().equal('user.name', 'foo')
    it('pass', () => {
      const result = isFoo
        .contextualize({
          user: { name: 'foo' }
  ...
  describe('propsEqual', () => {
    const sameName = Rule().propsEqual('subject.name', 'resource.name')
    const foo = { name: 'foo' }
    const bar = { name: 'bar' }

    it('pass', () => {
      sameName.contextualize({ subject: foo, resource: foo }).assert({})
    })
    ...
    it('left defined', () => {
      sameName.contextualize({ subject: foo }).assert({ resource: foo })
      const result = sameName.contextualize({ subject: foo }).test({})

      assert(!result)
    })

    it('right defined', () => {
      sameName.contextualize({ resource: foo }).assert({ subject: foo })
      const result = sameName
  ...
  describe('not', () => {
    const isSenior = Rule()
      .not()
      .equal('person.senior', false)
    const senior = is => ({ person: { senior: is } })

    it('pass', () => {
      isSenior.contextualize(senior(true)).assert({})
    })

    it('fail', () => {
      const result = isSenior.contextualize(senior(false)).test({})
      assert(!result)
    })

    it('undefined', () => {
      const result = isSenior.contextualize({}).test(senior(false))
      assert(!result)
      const result2 = isSenior.contextualize({}).test(senior(true))
      assert(result2)
    })
  })
